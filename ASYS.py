...
def SerialCOM(text):                #Communicates Commands
    text = text + "\r\n"
    ser.write(text)
    data = ""
    time.sleep(2)
...
def GPSInfo(bracket):                                   #Fetches GPS info
...
def Calc():               #Calculates Bearing and Distance
    x1 = math.radians(float(GPSInfo(3)))    #fetches longitude from GPS
    y1 = math.radians(float(GPSInfo(4)))    #same, but latitude
    x2 = math.radians(float(SMSParser()[0]))
    y2 = math.radians(float(SMSParser()[1]))
